...
function MockupScreen({ mockup, id, t }: { mockup: MockupContent; id: string; t: TFn }) {
  const base = `items.${id}.mockup`;
        ...
        <div className="bg-white rounded-2xl p-3 w-full shadow-lg">
          ...
          <div className="flex gap-1.5 mt-3">
            <div className="flex-1 h-7 rounded-lg bg-[#e0e0e0] flex items-center justify-center">
              <span className="text-black/70 text-[8px] font-bold uppercase">{t(`${base}.cancelLabel`)}</span>
            </div>
            <div className="flex-1 h-7 rounded-lg bg-mint flex items-center justify-center">
              <span className="text-black text-[8px] font-bold uppercase">{t(`${base}.confirmLabel`)}</span>
            </div>
          </div>
        </div>
      </div>
  ...
  if (mockup.kind === "chat") {
    const parts = t(`${base}.message`).split(/(bit\.ly\/\S+)/g);
    return (
    ...
    return (
      <div className="flex-1 bg-[#111] flex flex-col items-stretch justify-start gap-2 p-2.5 pt-3">
        {mockup.items.map((item, idx) => {
          const lines = t.raw(`${base}.items.${idx}.lines`) as string[];
          return (
            <div key={item.title} className="bg-white rounded-xl p-2.5 shadow">
              <p className="text-black text-[9px] font-bold uppercase tracking-wide mb-1">{t(`${base}.items.${idx}.title`)}</p>
              {lines.map((line) => (
              ...
              <button className="mt-1.5 w-full h-6 rounded-lg bg-mint flex items-center justify-center">
                <span className="text-black text-[8px] font-bold uppercase">{t(`${base}.items.${idx}.cta`)}</span>
              </button>
            </div>
          );
        })}
  ...
  if (mockup.kind === "data-rewards") {
    return (
      <div className="flex-1 bg-[#111] flex items-center justify-center p-3">
        <div className="bg-white rounded-2xl p-3 w-full shadow-lg">
          <p className="text-black text-[9px] text-center font-semibold mb-1">{t("dataFinished")}</p>
          <p className="text-black/60 text-[8px] text-center leading-snug">{t(`${base}.body`)}</p>
          <div className="flex gap-1.5 mt-3">
            <div className="flex-1 h-7 rounded-lg bg-[#e0e0e0] flex items-center justify-center">
              <span className="text-black/70 text-[8px] font-bold uppercase">{t(`${base}.cancelLabel`)}</span>
            </div>
...
function PhoneMockup({ mockup, id, t, isActive }: { mockup: MockupContent; id: string; t: TFn; isActive: boolean }) {
  return (
    <div
    ...
    >
      <div className="absolute inset-0 rounded-[2.5rem] border-[3px] border-white/20 bg-surface overflow-hidden shadow-2xl flex flex-col">
        ...
        {/* Screen */}
        <MockupScreen mockup={mockup} id={id} t={t} />
      </div>
    </div>
  );
}
          ...
          >
            {techServices.map((s, i) => (
              <SwiperSlide key={s.id} className="w-auto!" onClick={() => goTo(i)}>
                <PhoneMockup mockup={s.mockup} id={s.id} t={t} isActive={i === activeIndex} />
              </SwiperSlide>
            ))}
          </Swiper>
